...
# ve zikzak PATH görevi başlar.
CENTER_TOLERANCE_X = 40
CENTER_TOLERANCE_Y = 40

# Zikzak noktaları birbirine yakın olduğu için sabit 75 piksel yerine
# pencere boyutuyla ölçeklenen normalize tolerans kullanılır. Aktif hedefin
# tamamlanması için bu toleransın 0,50 saniye boyunca kesintisiz korunması
# gerekir. Merkezleme aşamasında ise bekleme süresi kullanılmaz.
...
# Gazebo kodundaki ileri-geri yardımcı yerel konum kontrolü.
...
# Canlı OpenCV kamera ve maske pencereleri gösterilir.
# Algılanan pencere üzerinde 7 noktalı zikzak rota, hedef noktaları
# ve o anda aktif olan hedef görsel olarak işaretlenir.
SHOW_WINDOWS = True
DISPLAY_W = 640
DISPLAY_H = 480
...
LOG_FILE = LOG_DIR / ("flight_log_zigzag_" + RUN_STAMP + ".csv")


# Pencere yüzeyini 7 hedef noktasıyla tarayan zikzak rota.
# Başlangıç pencere merkezidir. Rota sırasıyla:
# 1 sol orta, 2 sol alt, 3 sağ alt, 4 sağ orta,
# 5 sol orta, 6 sol üst ve 7 sağ üst noktalarına gider.
PATH_NORMALIZED = [
...
def forward_hold_velocity():
    """GUIDED girişindeki ileri-geri konumu düşük hızla korur."""
    if (
        forward_hold_x is None
        or forward_hold_y is None
        or forward_hold_yaw is None
        or current_x is None
        or current_y is None
    ):
        return 0.0

    delta_x = current_x - forward_hold_x
    delta_y = current_y - forward_hold_y
    forward_displacement = (
        delta_x * math.cos(forward_hold_yaw)
        + delta_y * math.sin(forward_hold_yaw)
    )
    forward_error = -forward_displacement

    return limit(
        FORWARD_HOLD_KP * forward_error,
        MAX_FORWARD_SPEED,
    )
...
def reset_path_hold():
    """PATH hedef toleransında geçen süreyi sıfırlar."""
    global path_hold_start
    global path_hold_elapsed

    path_hold_start = None
...
def get_segment_type(vision_mode, path_index):
    """Aktif zikzak hedefe giden ana hareket eksenini belirler.

    İlk hedef için başlangıç noktası pencere merkezi (0.50, 0.50) kabul
    edilir. Sonraki hedeflerde önceki normalize hedef ile aktif hedef
    arasındaki büyük değişim hangi eksendeyse o segment seçilir.
    """
    if vision_mode == "CENTER":
        return "CENTER"
    if vision_mode != "PATH" or not PATH_NORMALIZED:
        return "HOLD"

    index = max(0, min(int(path_index), len(PATH_NORMALIZED) - 1))
    target_u, target_v = PATH_NORMALIZED[index]

    if index == 0:
        previous_u, previous_v = 0.50, 0.50
    else:
        previous_u, previous_v = PATH_NORMALIZED[index - 1]
...
def draw_zigzag_path(frame, box, vision_mode, path_index):
    """7 noktalı zikzak rotayı algılanan pencere üzerinde çizer.

    Kırmızı çizgiler planlanan zikzak yolunu, yeşil noktalar tamamlanan
    hedefleri, sarı nokta aktif hedefi ve kırmızı noktalar bekleyen hedefleri
    gösterir. CENTER aşamasında bütün hedefler kırmızı gösterilir.
    """
    if box is None or not PATH_NORMALIZED:
        return

    path_pixels = [
        normalized_to_pixel(point_norm, box)
        for point_norm in PATH_NORMALIZED
    ]

    # Planlanan zikzak yolunun tamamını çiz.
    ...
    # PATH aşamasında merkezden ilk hedefe giden giriş segmentini göster.
    if vision_mode in ("PATH", "DONE"):
        center_pixel = normalized_to_pixel((0.50, 0.50), box)
        cv2.line(frame, center_pixel, path_pixels[0], (0, 255, 255), 2)

    # Aktif segmenti daha kalın sarı çizgiyle vurgula.
    if vision_mode == "PATH":
        active_index = max(0, min(int(path_index), len(path_pixels) - 1))
        if active_index == 0:
            segment_start = normalized_to_pixel((0.50, 0.50), box)
        else:
            segment_start = path_pixels[active_index - 1]
        cv2.line(
            frame,
            segment_start,
            path_pixels[active_index],
            (0, 255, 255),
            3,
        )

    for idx, (px, py) in enumerate(path_pixels):
        if vision_mode == "DONE":
            color = (0, 255, 0)
            radius = 5
        elif vision_mode == "PATH" and idx < path_index:
            color = (0, 255, 0)
            radius = 5
        elif vision_mode == "PATH" and idx == path_index:
            color = (0, 255, 255)
...
# PATH hedefinin tolerans içerisinde kesintisiz kaldığı süre.
# CENTER aşamasında süre koşulu kullanılmaz.
path_hold_start = None
path_hold_elapsed = 0.0
...
# GUIDED moda girildiği anda alınan ileri-geri yerel konum referansı.
forward_hold_x = None
forward_hold_y = None
forward_hold_yaw = None
...
print("Gazebo V13 uyumlu kontrol, telemetri ve 7 noktalı ZIGZAG PATH verileri CSV loguna kaydedilecek.")
...
print("Merkez toleransi: 40x40 px; merkezde bekleme yok.")
print(
    "Zikzak PATH hedef toleransi:",
    PATH_TOLERANCE_NORM,
    "normalize; hedefte bekleme:",
    PATH_HOLD_TIME,
    "s",
)
...
try:
    while True:
        ...
        if guided_active and not previous_guided_active:
            if local_position_received and attitude_received:
                forward_hold_x = current_x
                forward_hold_y = current_y
                forward_hold_yaw = current_yaw
            else:
                forward_hold_x = None
                forward_hold_y = None
                forward_hold_yaw = None
        ...
        # GUIDED giriş anında telemetri henüz gelmediyse ilk geçerli
        # yerel konum ve yaw bilgisi ileri-geri referansı olarak alınır.
        if (
            guided_active
            and forward_hold_x is None
            and local_position_received
            and attitude_received
        ):
            forward_hold_x = current_x
            forward_hold_y = current_y
            forward_hold_yaw = current_yaw
        ...
        # Gazebo uyumlu görev/log alanları. CENTER aşamasında bekleme yoktur.
        # PATH aşamasında hedef toleransının kesintisiz korunma süresi kaydedilir.
        ...
        if window_found:
            ...
            if vision_mode == "CENTER":
                ...
                # Hedefe ulaşma kararı normalize pencere koordinatlarında
                # verilir. Toleransın 0,50 saniye boyunca kesintisiz korunması
                # durumunda bir sonraki hedefe geçilir.
                if actual_u is not None and actual_v is not None:
                    path_inside_tolerance = (
                        abs(actual_u - target_u) <= PATH_TOLERANCE_NORM
                        and abs(actual_v - target_v) <= PATH_TOLERANCE_NORM
                    )
                else:
                    path_inside_tolerance = False

                if path_inside_tolerance:
                    if path_hold_start is None:
                        path_hold_start = loop_monotonic

                    path_hold_elapsed = loop_monotonic - path_hold_start
                    status = (
                        "PATH BEKLE "
                        + f"{path_hold_elapsed:.2f}/{PATH_HOLD_TIME:.2f} s"
                    )

                    if path_hold_elapsed >= PATH_HOLD_TIME:
                        ...
                        if path_index < len(PATH_NORMALIZED) - 1:
                            ...
                            reset_horizontal_derivative()
                            reset_path_hold()
                            status = "PATH BITTI - POMPA " + (
                                "ACIK" if pump_on else "KAPALI"
                            )
                else:
                    reset_path_hold()
        ...
        if SHOW_WINDOWS:
            if window_found and smoothed_box is not None:
                draw_zigzag_path(
                    frame,
                    smoothed_box,
                    vision_mode,
