...
class WindowOffboard(Node):
    ...
    # Dikdörtgenin uzun yatay kenarlarına özel fren.
    RECT_SIDE_KD = 0.00075
    RECT_MAX_SIDE_SPEED = 0.090
    RECT_NEAR_DISTANCE_PX = 60.0
    RECT_FINAL_DISTANCE_PX = 28.0
    RECT_NEAR_SPEED = 0.055
    RECT_FINAL_SPEED = 0.030
    ...
    def __init__(self):
        ...
        log_name = (
            'gazebo_flight_log_rectangle_5point_v14_'
        ...
        self.get_logger().info(
            'Controller started: V14 5-point rectangle segment-based control'
        )
        self.get_logger().info(
    ...
    def segment_type(self):
        ...
        # Kamera düğümü aktif dikdörtgen hedefinin ana hareket eksenini
        ...
        return 'VERTICAL' if self.path_index in (0, 2, 4) else 'HORIZONTAL'
    ...
    def horizontal_parameters(self):
        ...
        if (
            ...
            and self.path_index in (1, 3)
        ):
            active_kd = self.RECT_SIDE_KD
            absolute_error = abs(self.error_x)

            if absolute_error <= self.RECT_FINAL_DISTANCE_PX:
                speed_limit = self.RECT_FINAL_SPEED
                brake_zone = 'RECT_FINAL'
            elif absolute_error <= self.RECT_NEAR_DISTANCE_PX:
                speed_limit = self.RECT_NEAR_SPEED
                brake_zone = 'RECT_NEAR'
            else:
                speed_limit = self.RECT_MAX_SIDE_SPEED
                brake_zone = 'RECT_FAR'
